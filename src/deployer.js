...
import { parseEnvFile, serializeEnvFile } from './utils/env-parser.js';
...
export class Deployer {
  ...
  async create(name, options) {
    ...
    try {
      ...
      // Detect PHP version for Laravel projects
      let phpVersion = '8.3';
      if (projectType === 'laravel') {
        phpVersion = options.phpVersion || await this.detectPhpVersion(releaseDir);
        spinner.text = `Using PHP ${phpVersion}`;
      }
      ...
      // Generate environment file (merges with .env.example if present)
      spinner.text = 'Generating environment...';
      await this.generateEnvFile(projectDir, projectType, dbCredentials, options, releaseDir);

      // Copy merged .env to release directory for build
      ...
      await this.setupDockerConfig(projectDir, releaseDir, projectType, name, options, phpVersion);
      ...
      const metadata = {
        ...
        phpVersion: projectType === 'laravel' ? phpVersion : undefined,
  ...
  async update(name, options) {
    ...
    try {
      ...
      // Merge .env with new .env.example (preserves credentials, adds new keys)
      spinner.text = 'Merging environment files...';
      await this.mergeEnvForUpdate(projectDir, newReleaseDir, metadata.type);

      // Detect PHP version for Laravel projects
      let phpVersion = '8.3';
      if (metadata.type === 'laravel') {
        phpVersion = metadata.phpVersion || await this.detectPhpVersion(newReleaseDir);
        spinner.text = `Using PHP ${phpVersion}`;
      }
      ...
      if (await fs.pathExists(dockerfileSrc)) {
        ...
        await fs.writeFile(path.join(newReleaseDir, 'Dockerfile'), dockerfileContent);
      }
  ...
  async detectPhpVersion(projectDir) {
    const composerPath = path.join(projectDir, 'composer.json');
    if (!await fs.pathExists(composerPath)) return '8.3';

    try {
      const composer = await fs.readJson(composerPath);
      const phpConstraint = composer?.require?.php;
      if (!phpConstraint) return '8.3';

      const versionMatch = phpConstraint.match(/(\d+\.\d+)/);
      return versionMatch ? versionMatch[1] : '8.3';
    } catch (error) {
      console.warn('Failed to parse composer.json, using default PHP 8.3');
      return '8.3';
    }
  }

  async generateEnvFile(projectDir, projectType, dbCredentials, options, releaseDir = null) {
    const sharedDir = path.join(projectDir, 'shared');
    const envPath = path.join(sharedDir, '.env');

    // Step 1: Read .env.example from release directory if available
    // Using direct read with error handling to avoid TOCTOU race condition
    let baseEnv = Object.create(null);
    if (releaseDir) {
      const envExamplePath = path.join(releaseDir, '.env.example');
      try {
        const envExampleContent = await fs.readFile(envExamplePath, 'utf8');
        baseEnv = parseEnvFile(envExampleContent);
      } catch (error) {
        // ENOENT means file doesn't exist, which is fine
        if (error.code !== 'ENOENT') {
          console.warn(`Warning: Could not read .env.example: ${error.message}`);
        }
        // Continue with empty baseEnv
      }
    }

    // Step 2: Build deployer-generated values (these take precedence)
    const deployerEnv = {
      NODE_ENV: 'production'
    };

    if (projectType === 'laravel') {
      deployerEnv.APP_ENV = 'production';
      deployerEnv.APP_DEBUG = 'false';
      deployerEnv.APP_KEY = `base64:${this.generateRandomKey()}`;
    }

    if (dbCredentials) {
      if (projectType === 'laravel') {
        deployerEnv.DB_CONNECTION = dbCredentials.type === 'postgres' ? 'pgsql' : 'mysql';
        deployerEnv.DB_HOST = dbCredentials.host;
        deployerEnv.DB_PORT = String(dbCredentials.port);
        deployerEnv.DB_DATABASE = dbCredentials.database;
        deployerEnv.DB_USERNAME = dbCredentials.username;
        deployerEnv.DB_PASSWORD = dbCredentials.password;
      } else {
        deployerEnv.DATABASE_URL = dbCredentials.url;
      }
    }

    // Step 3: Merge - deployer values override base (from .env.example)
    const mergedEnv = { ...baseEnv, ...deployerEnv };

    // Step 4: Write merged result
    const envContent = `# Generated by Deployer\n${serializeEnvFile(mergedEnv)}`;
  ...
  /**
   * Merge existing .env with new .env.example during updates
   * Preserves existing credentials while adding new keys from .env.example
   *
   * @param {string} projectDir - Project directory path
   * @param {string} releaseDir - New release directory path
   * @param {string} projectType - Project type (e.g., 'laravel')
   */
  async mergeEnvForUpdate(projectDir, releaseDir, projectType) {
    const sharedEnvPath = path.join(projectDir, 'shared', '.env');
    const envExamplePath = path.join(releaseDir, '.env.example');

    // Read existing shared/.env (contains real credentials)
    // Using direct read with error handling to avoid TOCTOU race condition
    let existingEnv = Object.create(null);
    try {
      const existingContent = await fs.readFile(sharedEnvPath, 'utf8');
      existingEnv = parseEnvFile(existingContent);
    } catch (error) {
      if (error.code !== 'ENOENT') {
        console.warn(`Warning: Could not read existing .env: ${error.message}`);
      }
    }

    // Read new .env.example (may have new keys)
    let exampleEnv = Object.create(null);
    try {
    ...
    // Merge: existing values take precedence (preserves credentials)
    // New keys from .env.example are added with their placeholder values
    const mergedEnv = { ...exampleEnv, ...existingEnv };

    // Write merged result to shared/.env
    const envContent = `# Generated by Deployer\n${serializeEnvFile(mergedEnv)}`;
    await fs.writeFile(sharedEnvPath, envContent);

    // Copy to release directory for build
    await fs.copy(sharedEnvPath, path.join(releaseDir, '.env'));
  }
